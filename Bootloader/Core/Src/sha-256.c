 ...
 */
static inline void
consume_chunk (uint32_t *h, const uint8_t *p)
{
    unsigned i, j;
    uint32_t ah[8];
    ...
    /*
     * The w-array is really w[64], but since we only need 16 of them at a
     * time, we save stack by calculating 16 at a time.
     *
...
void
sha_256_init (struct Sha_256 *sha_256, uint8_t hash[SIZE_OF_SHA_256_HASH])
{
    sha_256->hash = hash;
...
void
sha_256_write (struct Sha_256 *sha_256, const void *data, size_t len)
{
    sha_256->total_len += len;

    /*
     * The following cast is not necessary, and could even be considered as
     * poor practice. However, it makes this file valid C++, which could be a
     * good thing for some use cases.
     */
    ...
    while (len > 0)
    {
         ...
         * chunk size, no copies are necessary. We operate directly on the
         * input data instead.
         */
...
uint8_t *
sha_256_close (struct Sha_256 *sha_256)
{
    ...
    *pos++ = 0x80;
     ...
     * last chunk, and zero padding before that. But we do not necessarily have
...
void
calc_sha_256 (uint8_t hash[SIZE_OF_SHA_256_HASH], const void *input,
              size_t len)
{
